                                ...
                                <tbody>
                                    @foreach ($requests as $index => $req)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($req->rmstatus === 'rmapprove' || $req->rmstatus === 'rmreject')
                                                    <form action="{{ route('hrm.mapprove.permanent', $req->id) }}"
                                                        method="POST" style="display:inline-block;">
                                                        @csrf

                                                        @if ($req->mstatus === 'mpending')
                                                            <button type="submit" name="action" value="mapprove"
                                                    ...
                                                    <!-- Modal only if mstatus is pending -->
                                                    @if ($req->mstatus === 'mpending')
                                                        <div class="modal fade" id="rejectModal{{ $req->id }}"
                                                            tabindex="-1"
                                                            aria-labelledby="rejectModalLabel{{ $req->id }}"
                                                            aria-hidden="true">
                                                            <div class="modal-dialog">
                                                                <form method="POST"
                                                                    action="{{ route('hrm.mapprove.permanent', $req->id) }}">
                                                                    @csrf
                                                                    <input type="hidden" name="action" value="mreject">
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            <h5 class="modal-title"
                                                                                id="rejectModalLabel{{ $req->id }}">
