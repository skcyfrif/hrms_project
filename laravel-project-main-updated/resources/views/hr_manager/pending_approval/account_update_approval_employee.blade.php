    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h5 class="text-center mb-4">Update Account Profile Approval Of All Reporting Manager</h5>
                                ...
                                <tbody>
                                    @forelse ($requests as $index => $reques)
                                        <tr>
                                            ...
                                            <td>
                                                @php $status = strtolower($reques->m_status); @endphp
                                                @if ($status === 'mpending')
                                            ...
                                            <td>
                                                @if ($status === 'mpending')
                                                    <form action="{{ route('hrm.accountrequest.approve', $reques->id) }}"
                                                        method="POST" style="display:inline-block">
                                                        @csrf
                                                        <button type="submit"
                                                            class="btn btn-sm btn-success">Approve</button>
                                                    </form>
                                                    <form action="{{ route('hrm.accountrequest.reject', $reques->id) }}"
                                                        method="POST" style="display:inline-block">
                                                        @csrf
