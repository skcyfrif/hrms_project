                            ...
                            <table id="dataTableExample" class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Total Days</th>
                                        <th>Reason</th>
                                        <th>M Leave Status</th>
                                        <th>RM Leave Status</th>
                                        <th>File</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach ($approvals as $key => $leave)
                                        <tr>
                                            ...
                                            @if ($leave->m_status == 'mpending')
                                                <a href="{{ route('mapprove.leave', $leave->id) }}" class="btn btn-inverse-success">Approve</a>
                                                <button class="btn btn-inverse-danger rejectBtn" data-id="{{ $leave->id }}" data-employee="{{ $leave->employeeleavestatusinhrm->employee_id }}">Reject</button>
                                            @endif
                                        </td> --}}
                                            <td>
                                                @if ($leave->m_status == 'mapprove')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="rejectLeaveForm" method="POST" action="{{ route('mreject.leave.submit') }}">
                        @csrf
                        <input type="hidden" name="leave_id" id="leave_id">
                        <input type="hidden" name="employee_id" id="employee_id">
                        <input type="hidden" name="rejected_by" value="{{ auth()->user()->id }}">
                        <input type="hidden" name="status" value="mreject">
